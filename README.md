...
- 自动执行返回失败或抛出异常时，会自动转人工工单；工单完结后仍通过反馈同步回流到知识库。
...
- PostgreSQL 知识库采用两层结构：`kb_fault_patterns` 存储归一化后的故障模式，`kb_incident_cases` 存储具体告警/Jira 事件案例；多个相似事件会关联到同一个故障模式。
- 旧版扁平表 `kb_experiences` 已移除；初始化脚本和运行时 schema 检查会删除该表，新的检索与回流只使用故障模式和事件案例表。
- 当前 embedding 为项目内置的确定性本地向量化（无外部模型依赖），用于快速可运行验证。
- 运维经验写入知识库时会先归一化根因并生成故障模式去重键，重复工单案例会被自动跳过，不会持续灌入相同知识。
- 对“近似重复”故障模式，系统会在入库前结合向量距离和文本重合度做二次判定，避免同义改写反复生成新模式。
